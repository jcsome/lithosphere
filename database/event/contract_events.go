...
var (
	TransferEventABIHash = "0xddf252ad1be2c89b69c2b068fc378daa952ba7f163c4a11628f55a4df523b3ef"
)
...
type ContractEventsView interface {
	...
	L1ContractEventsWithContractFilter(string, string, int64, int64) ([]L1ContractEvent, error)
	...
	L2ContractEventsWithContractFilter(string, string, int64, int64) ([]L2ContractEvent, error)
...
func (db *contractEventsDB) L1ContractEventsWithContractFilter(add string, eventSignature string, fromTimestamp int64, toTimestamp int64) ([]L1ContractEvent, error) {
	var events []L1ContractEvent
	result := db.gorm.Table("l1_contract_events").Where("timestamp > ? and timestamp < ? and contract_address = ? and event_signature = ?", fromTimestamp, toTimestamp, add, eventSignature).Find(&events)
	if result.Error != nil {
		if errors.Is(result.Error, gorm.ErrRecordNotFound) {
			return nil, nil
		}
...
func (db *contractEventsDB) L2ContractEventsWithContractFilter(add string, eventSignature string, fromTimestamp int64, toTimestamp int64) ([]L2ContractEvent, error) {
	var events []L2ContractEvent
	result := db.gorm.Table("l2_contract_events").Where("timestamp > ? and timestamp < ? and contract_address = ? and event_signature = ?", fromTimestamp, toTimestamp, add, eventSignature).Find(&events)
	if result.Error != nil {
		if errors.Is(result.Error, gorm.ErrRecordNotFound) {
			return nil, nil
		}
